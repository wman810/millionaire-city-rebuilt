   ...
   public class GTween extends EventDispatcher
   {
      ...
      public function set position(param1:Number) : void
      {
         ...
         else
         {
            ...
            if(this.reflect && Boolean(this._position / this.duration & 1))
            {
               this.calculatedPosition = this.duration - this.calculatedPosition;
            }
         }
         this.ratio = this.duration == 0 && this._position >= 0 ? 1 : Number(this.ease(this.calculatedPosition / this.duration,0,1,1));
         if(Boolean(this.target) && (Boolean(this._position >= 0 || this.positionOld >= 0)) && this.calculatedPosition != this.calculatedPositionOld)
         {
            ...
            {
               _loc5_ = Number(this._initValues[_loc4_]);
               _loc6_ = Number(this._rangeValues[_loc4_]);
               ...
               if(_loc8_)
               {
                  ...
                  {
                     _loc7_ = Number(_loc8_[_loc10_].tween(this,_loc4_,_loc7_,_loc5_,_loc6_,this.ratio,_loc3_));
                     _loc10_++;
                  }
                  if(!isNaN(_loc7_))
                  {
      ...
      public function init() : void
      {
         ...
         for(_loc1_ in this._values)
         {
            if(plugins[_loc1_])
            {
               ...
               _loc4_ = _loc1_ in this.target ? Number(this.target[_loc1_]) : NaN;
               _loc5_ = 0;
               while(_loc5_ < _loc3_)
               {
                  _loc4_ = Number(_loc2_[_loc5_].init(this,_loc1_,_loc4_));
                  _loc5_++;
               }
               if(!isNaN(_loc4_))
               {
...
dynamic class TargetProxy extends Proxy
{
   ...
   {
      var _loc2_:Number = Number(this.tween.getValue(param1));
